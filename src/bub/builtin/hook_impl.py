...
@hookimpl
def normalize_inbound(message: Envelope) -> Envelope:
...
@hookimpl
def resolve_session(message: Envelope) -> str:
...
@hookimpl
def build_prompt(message: Envelope, session_id: str, state: State) -> str:
    _ = session_id
...
@hookimpl
async def run_model(prompt: str, session_id: str, state: State) -> str | None:
