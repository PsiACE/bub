...
`bub` currently exposes five builtin commands: `run`, `gateway`, `chat`, `auth`, and the hidden compatibility command `message`.
...
```bash
uv run bub auth login openai --manual --no-browser
```
